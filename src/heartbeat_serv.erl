...
handle_info(heartbeat, #state{maxusr=MaxUsers, name=Name, salt=Salt, users=Users} = State)
	...
	,  {noreply, State}
	;
handle_info({Ref, join, updates, Pids}, #state{ref=Ref, users=Users} = State)  -> {noreply, State#state{users=Users+length(Pids)}};
handle_info({Ref, leave, updates, Pids}, #state{ref=Ref, users=Users} = State) -> {noreply, State#state{users=Users-length(Pids)}};
