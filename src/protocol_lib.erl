...
parse_toc(<<16#00, 16#7, Name:?STR_SIZE/binary, Key:?STR_SIZE/binary, IsOp, Rest/binary>>, Acc) ->
	parse_toc(Rest, [{id, util_lib:bin_trim_right(Name), util_lib:bin_trim_right(Key), toOp(IsOp)} | Acc]);
...
parse_toc(<<16#0d, PlayerId/signed, Message:?STR_SIZE/binary, Rest/binary>>, Acc) ->
	parse_toc(Rest, [{msg, PlayerId, util_lib:bin_trim_right(Message)} | Acc]);
...
build({msg, PlayerId, Message})
	-> PaddedMessage = util_lib:bin_pad(Message)
